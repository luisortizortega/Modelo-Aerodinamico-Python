...
def calcular_parametros_comparacion(M_ref, coef_ref, interpolador_modelo): #función para calcular el RMSE y R^2 comparando el modelo con una serie de datos de referencia que se proporcionen
...
def generar_exportacion_grafica(mach_simulado, curva_cl_base, curva_cd_base, lim_mach_cl_base,
    ...
    if not es_caso_extremo: #si no hay caso extremo entonces se dibuja la curva teórica (base) en azul hasta el límite de mach
        ...
        grafico_sustentacion.plot(mach_simulado[filtro_mach_base], curva_cl_base[filtro_mach_base], 'b-', lw=2.5, label='Modelo propio')
...
def main(): #la función main es la principal, que es la que da las órdenes para coordinar el resto de funciones, primero lee los datos, los valida y prepara, luego calcula, luego construye y genera la gráfica
    ...
    if archivo_ref_cl: #se comprueba si se guardó el nombre del archivo de referencia para cl
        ...
        if M_ref_cl is not None: #se calculan las métricas de cl si hay datos de referencia cargados
            ...
            if not es_caso_extremo: #se comprueba si la teoría sigue disponible o no
                met_cl_base = calcular_parametros_comparacion(M_ref_cl, cl_ref, modelo_cl_base) #se llama a la función para calcular los parámetros r^2 y rmse y devuelve los mismos
            ...
            if modelo_cl_usuario is not None: #se comprueba si el usuario puso puntos opcionales
                met_cl_usuario = calcular_parametros_comparacion(M_ref_cl, cl_ref, modelo_cl_usuario) #devuelve los parámetros para la comparación pero con la curva de datos del usuario
    ...
    if archivo_ref_cd: #el funcionamiento es igual que para el cl en las líneas anteriores
        ...
        if M_ref_cd is not None: #se calculan las métricas de cd si hay datos de referencia cargados
            print("\n Parámetros cd")
            if not es_caso_extremo:
                met_cd_base = calcular_parametros_comparacion(M_ref_cd, cd_ref, modelo_cd_base)
                if met_cd_base: print(f"    TEORÍA  -> RMSE: {met_cd_base['rmse']:.5f} | R²: {met_cd_base['r2']:.4f}")

            if modelo_cd_usuario is not None:
                met_cd_usuario = calcular_parametros_comparacion(M_ref_cd, cd_ref, modelo_cd_usuario)
    ...
    if M_ref_cl is not None or M_ref_cd is not None:
        grafica_comparacion(M_ref_cl, cl_ref, M_ref_cd, cd_ref, modelo_cl_base, modelo_cd_base,
            modelo_cl_usuario, modelo_cd_usuario, met_cl_base, met_cd_base, met_cl_usuario, met_cd_usuario, #se pasan las métricas a la gráfica
