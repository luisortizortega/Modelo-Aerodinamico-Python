...
def cargar_datos_entrada(ruta_archivo):  #función para cargar los datos de entrada al modelo y si no se encuentra el archivo datos_entrada.txt salta un error y se para el programa
    ...
    with open(ruta_archivo, 'r', encoding='utf-8') as archivo:  #with asegura que lo que pase dentro de este bloque termine cerrando el archivo que se va a leer. El open(...) busca el archivo utilizando la dirección de la carpeta guardada en la variable ruta_archivo, o sea, busca datos_entrada en la carpeta del main creada llamada archivos_configuracion. r es para leer solamente
        for num_linea, linea in enumerate(archivo, 1): #este bucle va a leer el archivo de forma que toma el texto de la línea y le asigna un número de orden a esa línea que ha leído empezando por el 1, la primera línea será la 1. num_linea guarda el número entero actual mientras se recorre el archivo y linea guarda el texto correspondiente
            ...
            if clave in ['ARCHIVO_REF_CL', 'ARCHIVO_REF_CD']: #lee los archivos de validación en caso de querer hacerla
...
def generar_puntos_cl_base(datos, cl0, alpha_ef_rad, es_grueso, es_limite_alpha): #función para crear la lista de puntos de cl de la curva teórica (base)
    ...
    if not es_grueso and not es_limite_alpha: #se comprueba que el perfil sea delgado y alpha dentro de los límites
        ...
        rango_supersonico = np.linspace(1.20, 1.80, 45) #la forma de proceder para supersónico es igual que para subsónico
        for m_actual in rango_supersonico:
    ...
    return np.array(puntos_mach), np.array(puntos_coeficientes) #la función devuelve las listas como un array porque son vectores que son con los que trabajan luego matplotlib y el interpolador
...
def generar_puntos_cl_con_usuario(datos, cl0, puntos_usuario): #función para crear los puntos de cl a partir de lo que introduzca el usuario
...
def generar_puntos_cd_con_usuario(datos, cd_minimo, puntos_usuario, es_romo): #función para crear los puntos de cd a partir de lo que introduzca el usuario
...
def calcular_parametros_validacion(M_ref, coef_ref, interpolador_modelo): #función para calcular el RMSE y R^2 comparando el modelo con una serie de datos de referencia que se proporcionen

    #se evalúa el modelo en los Mach de referencia para poder comparar cada punto con el del modelo en el mismo mach
    coef_modelo = interpolador_modelo(M_ref)
    
    #se eliminan los puntos donde el modelo no tiene datos (NaN)
    mascara = np.isfinite(coef_modelo) & np.isfinite(coef_ref) #se busca que el valor del modelo y de referencia sean finitos y válidos
    coef_modelo = coef_modelo[mascara] #se aplica el filtro a los coeficientes del modelo y de referencia
    coef_ref    = coef_ref[mascara]
    n = len(coef_ref) #se cuentan en 'n' el número de puntos válidos que han quedado
    
    if n < 2: #si solamente hay 2 puntos válidos entonces no se puede hacer la comparación
        print("Aviso para la validación: no hay suficientes puntos comparables.")
        return None
    
    #RMSE, diferencia de error entre el modelo y el real elevado al cuadrado, se calcula la media de esos errores al cuadrado y después la raíz
    rmse = np.sqrt(np.mean((coef_modelo - coef_ref)**2))
    
    #error relativo medio (%) (no utilizado finalmente)
    mascara_no_cero = np.abs(coef_ref) > 1e-8
    error_rel = np.mean(
        np.abs(coef_modelo[mascara_no_cero] - coef_ref[mascara_no_cero])
        / np.abs(coef_ref[mascara_no_cero])
    ) * 100.0
    
    #R^2, se calcula el coeficiente de determinación
    ss_res = np.sum((coef_ref - coef_modelo)**2) #esto es el error que el modelo no ha podido predecir
    ss_tot = np.sum((coef_ref - np.mean(coef_ref))**2) #esta es la varianza de los datos reales respecto a su media
    r2 = 1.0 - ss_res / ss_tot if ss_tot > 1e-12 else 0.0 #el else es por si todos los datos son el mismo número, la varianza daría 0 y se dividiría por 0 dando error
    
    return {'n': n, 'rmse': rmse, 'error_rel': error_rel, 'r2': r2} #se devuelven todos los cálculos en un diccionario con etiquetas con nombre de cada parámetro
...
def grafica_validacion(M_ref_cl, cl_ref, M_ref_cd, cd_ref, modelo_cl_base, modelo_cd_base, modelo_cl_usuario, modelo_cd_usuario, met_cl_base, met_cd_base, met_cl_usuario, met_cd_usuario, es_caso_extremo, alpha_grad, tc, carpeta, lim_mach_cl_base, lim_mach_cd_base, lim_mach_cl_opc, lim_mach_cd_opc):
    #función para generar una gráfica de validación superponiendo los datos de referencia con los modelos de teoría y/o usuario.
    ...
    if M_ref_cl is not None:
        ax.scatter(M_ref_cl, cl_ref, color='black', s=40, zorder=6, label='Datos de referencia', marker='o') #se dibujan los datos sueltos
        texto_cl = "" #se crea una variable de texto vacía donde se irán guardando los parámetros rmse y r^2
        ...
        if not es_caso_extremo:
            ...
            ax.plot(M_plot[filtro_base], cl_modelo, 'b-', lw=2.5, label='Modelo teórico') #se pinta en azul una línea del modelo teórico
            if met_cl_base:
                texto_cl += (f"TEORÍA:\n RMSE: {met_cl_base['rmse']:.4f} | R²: {met_cl_base['r2']:.4f}\n") #si se calcularon métricas como rmse o r^2 entonces se añaden a la variable textos_cl
                #el += significa añadir y guardar, o sea, se coge texto_cl y se le añade lo siguiente y se guarda, en el siguiente bucle coge todo lo que contiene texto_cl y añade lo siguiente, etc
        ...
        if modelo_cl_usuario is not None:
            ...
            etiqueta = 'Modelo usuario' if es_caso_extremo else 'Corrección usuario'
            ax.plot(M_plot[filtro_opc], cl_usuario, estilo, lw=2.5, label=etiqueta)
            if met_cl_usuario:
                if texto_cl: texto_cl += "\n"
                texto_cl += (f"USUARIO:\n RMSE: {met_cl_usuario['rmse']:.4f} | R²: {met_cl_usuario['r2']:.4f}")

        if texto_cl: #se coge todo el texto acumulado en texto_cl de rmse y r^2 y se pone en la gráfica
            ax.text(0.03, 0.97, texto_cl, transform=ax.transAxes, fontsize=9, va='top', ha='left', bbox=dict(boxstyle='round', facecolor='white', alpha=0.85))
                    
    ax.set_xlabel('Número de Mach ($M_\\infty$)', fontsize=13)
    ax.set_ylabel('Coeficiente $c_l$', fontsize=13)
    ax.set_title('Validación — Sustentación', fontsize=12)
    ax.set_xlim(0, 1.65)
    ax.xaxis.set_major_locator(MultipleLocator(0.2))
    ax.grid(True, alpha=0.35)
    ax.legend(fontsize=10, loc='upper right') #líneas para el dibujo de la gráfica de validación
    ...
    if M_ref_cd is not None:
        ax.scatter(M_ref_cd, cd_ref, color='black', s=40, zorder=6, label='Datos de referencia', marker='o')
        texto_cd = ""
        ...
        if not es_caso_extremo:
            ...
            ax.plot(M_plot[filtro_base], cd_modelo, 'r-', lw=2.5, label='Modelo teórico')
            if met_cd_base:
                texto_cd += (f"TEORÍA:\n RMSE: {met_cd_base['rmse']:.5f} | R²: {met_cd_base['r2']:.4f}\n")
        ...
        if modelo_cd_usuario is not None:
            ...
            if met_cd_usuario:
                if texto_cd: texto_cd += "\n"
                texto_cd += (f"USUARIO:\n RMSE: {met_cd_usuario['rmse']:.5f} | R²: {met_cd_usuario['r2']:.4f}")

        if texto_cd:
            ax.text(0.03, 0.97, texto_cd, transform=ax.transAxes, fontsize=9, va='top', ha='left', bbox=dict(boxstyle='round', facecolor='white', alpha=0.85))
                    
    ax.set_xlabel('Número de Mach ($M_\\infty$)', fontsize=13)
    ax.set_ylabel('Coeficiente $c_d$', fontsize=13)
    ax.set_title('Validación — Resistencia', fontsize=12)
    ax.set_xlim(0, 1.65)
    ax.set_ylim(bottom=0)
    ax.xaxis.set_major_locator(MultipleLocator(0.2))
    ax.grid(True, alpha=0.35)
    ax.legend(fontsize=10, loc='lower right') #líneas para el dibujo de la gráfica de cd de validación

    fig.suptitle(f'Validación del modelo frente a literatura — $t/c={tc:.2f}$, $\\alpha={alpha_grad:.1f}°$', fontsize=12)
    fig.tight_layout()
    ruta = os.path.join(carpeta, f'validacion_alpha{alpha_grad:.1f}_tc{tc:.2f}.png')
    fig.savefig(ruta, dpi=300)
    plt.close(fig)
    print(f"Gráfica de validación generada en: {ruta}")

def generar_exportacion_grafica(mach_simulado, curva_cl_base, curva_cd_base, lim_mach_cl_base, lim_mach_cd_base, nodos_mach_cl_base, nodos_c_cl_base, nodos_mach_cd_base, nodos_c_cd_base, curva_cl_opcional, curva_cd_opcional, lim_mach_cl_opc, lim_mach_cd_opc, nodos_mach_cl_opc, nodos_c_cl_opc, nodos_mach_cd_opc, nodos_c_cd_opc, existen_puntos_usuario_cl, existen_puntos_usuario_cd, valor_alpha, valor_espesor, es_borde_romo, ruta_directorio_salida):
 #esta función lo que hace es recoger las listas definitivas de mach, cd y cl y las prepara para exportar los datos en un csv y también para trazar las curvas
    ...
    if not es_caso_extremo: #si no hay caso extremo entonces se dibuja la curva teórica (base) en azul hasta el límite de mach
        ...
        grafico_sustentacion.plot(mach_simulado[filtro_mach_base], curva_cl_base[filtro_mach_base], 'b-', lw=2.5, label='Modelo teórico')
                                  #se dibuja la gráfica
        if lim_mach_cl_base < 1.55: #si la curva no llega hasta el final del eje de mach a simular, se dibuja una línea naranja discontinua marcando donde termina
            grafico_sustentacion.axvline(x=lim_mach_cl_base, color='orange', lw=1.2, ls=':', label=f'Límite de gráfica (M={lim_mach_cl_base:.2f})')
                                         
        filtro_puntos_base = nodos_mach_cl_base <= lim_mach_cl_base #aplica el filtro de que los puntos de mach sean inferiores al límite
        grafico_sustentacion.scatter(nodos_mach_cl_base[filtro_puntos_base], nodos_c_cl_base[filtro_puntos_base], color='blue', s=25, zorder=5, alpha=0.5, label='Puntos')
                                  #se dibujan los puntos en el gráfico por los que pasará el interpolador
    if existen_puntos_usuario_cl and curva_cl_opcional is not None: #si además de la teoría, o no hay teoría pero sí hay curva de puntos obligatorios y puntos opcionales de cl proporcionados, se dibuja en verde discontinua
        ...
        texto_leyenda_opc = 'Modelo solo datos de usuario' if es_caso_extremo else 'Corrección del usuario' #leyenda de la línea
        ...
        grafico_sustentacion.plot(mach_simulado[filtro_mach_opc], curva_cl_opcional[filtro_mach_opc], estilo_linea, lw=grosor_linea, label=texto_leyenda_opc)
                                 #se dibuja la gráfica 
        if len(nodos_mach_cl_opc) > 0: #si la lista de puntos opcionales es mayor que cero se dibujan los puntos sobre los que pasará la gráfica de puntos opcionales
            ...
            grafico_sustentacion.scatter(nodos_mach_cl_opc[filtro_nodos_opc], nodos_c_cl_opc[filtro_nodos_opc], color='green', s=40, zorder=6, alpha=0.8, marker='^', label='Datos aportados por usuario')
    ...
    grafico_sustentacion.axvline(x=1.0, color='gray', lw=1.0, ls='--', alpha=0.5, label='Barrera (M=1)')
    ...
    if not es_caso_extremo:
        ...
        etiqueta_cd_base = 'Modelo teórico'
        if es_borde_romo:
        ...
        if lim_mach_cd_base < 1.55: #funciona esto igual que para el cl
            grafico_resistencia.axvline(x=lim_mach_cd_base, color='orange', lw=1.2, ls=':', label=f'Límite de gráfica (M={lim_mach_cd_base:.2f})')
                                        
        filtro_puntos_base = nodos_mach_cd_base <= lim_mach_cd_base #filtro y dibujo de puntos en la gráfica teórica
        grafico_resistencia.scatter(nodos_mach_cd_base[filtro_puntos_base], nodos_c_cd_base[filtro_puntos_base], color='red', s=25, zorder=5, alpha=0.5, label='Puntos')
    ...
    if existen_puntos_usuario_cd and curva_cd_opcional is not None: #caso de puntos opcionales y, además, curva cd opcional
        ...
        texto_leyenda_opc = 'Modelo solo datos de usuario' if es_caso_extremo else 'Corrección del usuario'
        ...
        if len(nodos_mach_cd_opc) > 0:
            ...
            grafico_resistencia.scatter(nodos_mach_cd_opc[filtro_nodos_opc], nodos_c_cd_opc[filtro_nodos_opc], color='purple', s=40, zorder=6, alpha=0.8, marker='^', label='Datos aportados por usuario')
    ...
    grafico_resistencia.axvline(x=1.0, color='gray', lw=1.0, ls='--', alpha=0.5, label='Barrera (M=1)')
...
def main(): #la función main es la principal, que es la que da las órdenes para coordinar el resto de funciones, primero lee los datos, los valida y prepara, luego calcula, luego construye y genera la gráfica
    #inicialización de variables de validación vacías que se rellenarán si hay archivos de referencia para hacer la validación. Si no los hay se quedan vacías
    ...
    print("Realizando validación de los datos y condiciones...")
    ...
    vectores_mach_cl_base, vectores_coef_cl_base = generar_puntos_cl_base(parametros, cl_base_mach0, angulo_efectivo_rad, es_caso_grueso, es_caso_limite)
    vectores_mach_cd_base, vectores_coef_cd_base = generar_puntos_cd_base(parametros, cl_base_mach0, cd_polar_minimo, factor_forma_k, angulo_efectivo_rad, integral_espesor, es_caso_grueso, es_caso_limite, perfil_redondeado, es_viable_ecuacion_cd_sup, puntos_sup_cd_romo)
    ...
    nodos_limpios_cl_m, nodos_limpios_cl_c = preparar_nodos_interpolacion(vectores_mach_cl_base, vectores_coef_cl_base)
    nodos_limpios_cd_m, nodos_limpios_cd_c = preparar_nodos_interpolacion(vectores_mach_cd_base, vectores_coef_cd_base)
    #se llama a la función que prepara los puntos quitando duplicados, etc
    vectores_mach_cl_opc, vectores_coef_cl_opc = generar_puntos_cl_con_usuario(parametros, cl_base_mach0, lista_opcionales_cl)
    vectores_mach_cd_opc, vectores_coef_cd_opc = generar_puntos_cd_con_usuario(parametros, cd_polar_minimo, lista_opcionales_cd, perfil_redondeado)
    ...
    generar_exportacion_grafica(escala_mach, curva_suavizada_cl_base, curva_suavizada_cd_base, tope_mach_cl_base, tope_mach_cd_base, nodos_limpios_cl_m, nodos_limpios_cl_c, nodos_limpios_cd_m, nodos_limpios_cd_c, curva_suavizada_cl_usuario, curva_suavizada_cd_usuario, tope_mach_cl_usuario, tope_mach_cd_usuario, nodos_usuario_cl_m, nodos_usuario_cl_c, nodos_usuario_cd_m, nodos_usuario_cd_c, usuario_ingreso_cl, usuario_ingreso_cd_valido, angulo_ataque_geom, espesor_relativo, perfil_redondeado, directorio_resultados)
    #se llama a la función de gráficas pasando todos los datos necesarios calculados
    ...
    #si el usuario ha escrito el nombre del archivo de referencia para la validación, se guarda el nombre del archivo y si no se guarda un none para usar después este nombre para abrir el archivo
    ...
    if archivo_ref_cl: #se comprueba si se guardó el nombre del archivo de referencia para cl
        ...
        if M_ref_cl is not None:
            print("\n  [Validación cl]")
            if not es_caso_extremo: #se comprueba si la teoría sigue disponible o no
                met_cl_base = calcular_parametros_validacion(M_ref_cl, cl_ref, modelo_cl_base) #se llama a la función para calcular los parámetros r^2 y rmse y devuelve los mismos
                if met_cl_base: print(f"    TEORÍA  -> RMSE: {met_cl_base['rmse']:.4f} | R²: {met_cl_base['r2']:.4f}") #se muestra en el terminal los valores
            
            if modelo_cl_usuario is not None: #se comprueba si el usuario puso puntos opcionales
                met_cl_usuario = calcular_parametros_validacion(M_ref_cl, cl_ref, modelo_cl_usuario) #devuelve los parámetros para la validación pero con la curva de datos del usuario
                if met_cl_usuario: print(f"    USUARIO -> RMSE: {met_cl_usuario['rmse']:.4f} | R²: {met_cl_usuario['r2']:.4f}") #se muestra en el terminal los valores
    ...
    if archivo_ref_cd: #el funcionamiento es igual que para el cl en las líneas anteriores
        ...
        if M_ref_cd is not None:
            print("\n  [Validación cd]")
            if not es_caso_extremo:
                met_cd_base = calcular_parametros_validacion(M_ref_cd, cd_ref, modelo_cd_base)
                if met_cd_base: print(f"    TEORÍA  -> RMSE: {met_cd_base['rmse']:.5f} | R²: {met_cd_base['r2']:.4f}")
            
            if modelo_cd_usuario is not None:
                met_cd_usuario = calcular_parametros_validacion(M_ref_cd, cd_ref, modelo_cd_usuario)
                if met_cd_usuario: print(f"    USUARIO -> RMSE: {met_cd_usuario['rmse']:.5f} | R²: {met_cd_usuario['r2']:.4f}")

    if M_ref_cl is not None or M_ref_cd is not None: #se comprueba si hay datos para la validación y si los hay se llama a generar la gráfica
        grafica_validacion(M_ref_cl, cl_ref, M_ref_cd, cd_ref, modelo_cl_base, modelo_cd_base, modelo_cl_usuario, modelo_cd_usuario, met_cl_base, met_cd_base, met_cl_usuario, met_cd_usuario, es_caso_extremo, angulo_ataque_geom, espesor_relativo, directorio_resultados, tope_mach_cl_base, tope_mach_cd_base, tope_mach_cl_usuario, tope_mach_cd_usuario)
